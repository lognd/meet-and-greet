...
Thread safety: a single connection is shared across threads with check_same_thread=False.
A threading.Lock serialises ALL connection access because the Python sqlite3.Connection
object is not thread-safe: concurrent execute() calls on the same object corrupt
internal state even with WAL mode enabled at the SQLite level."""
...
class DataStore:
    ...
    def register_new_student(
        ...
        forename: str,
        surname: str,
        passphrase_pool: list[str],
    ) -> Student:
        """Pick an unused passphrase and insert the student atomically.

        This method holds the lock for the entire read-then-write sequence so
        that no two concurrent registrations can receive the same passphrase.
        """
        with self._lock:
            used_rows = self._conn.execute(
                "SELECT passphrase FROM students"
            ).fetchall()
            used = {r["passphrase"] for r in used_rows}
            available = [p for p in passphrase_pool if p not in used]
            if available:
                passphrase = random.choice(available)
            else:
                idx = len(used) - len(passphrase_pool)
                passphrase = (
                    f"{passphrase_pool[idx % len(passphrase_pool)]}"
                    f"-{idx // len(passphrase_pool) + 2}"
                )
            student = Student(
                uuid=uuid,
    ...
    def all_students(self) -> list[Student]:
        with self._lock:
            rows = self._conn.execute(
                "SELECT * FROM students ORDER BY registered_at"
            ).fetchall()
    ...
    def add_meeting_if_not_exists(self, meeting: Meeting) -> bool:
        """Insert meeting only if no record exists for this pair (either direction).

        Returns True if inserted, False if already recorded.
        Holds the lock for the entire check+insert to prevent double-recording.
        """
        with self._lock:
            existing = self._conn.execute(
                ...
                (
                    meeting.finder_uuid, meeting.target_uuid,
                    meeting.target_uuid, meeting.finder_uuid,
                ),
            ).fetchone()
            if existing:
                return False
